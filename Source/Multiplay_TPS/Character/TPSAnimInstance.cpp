...
void UTPSAnimInstance::NativeUpdateAnimation(float DeltaTime)
{
	...
	bUseAimOffset = character->GetCombatState() == ECombatState::ECS_Unoccupied && !character->GetDisableGameplay();
	bTransformRightHand = character->GetCombatState() == ECombatState::ECS_Unoccupied && !character->GetDisableGameplay();
}
